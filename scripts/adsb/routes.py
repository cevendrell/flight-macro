...
def main() -> int:
    ...
    if not prefixes:
        log("no callsigns in the snapshots yet — nothing to look up")
        return 0
    ...
    log(f"{files} tables, {len(routes):,} routes loaded; no table for: {', '.join(missing) or '—'}")

    # Keep only what we have heard. The full set is a few hundred thousand
    # rows; the site never needs a route for a callsign it has not recorded.
